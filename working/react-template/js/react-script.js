  ...
  const options = {
    actions: {
      ...
      calculate:              assign({data:     (ctx,evt) => calculate(ctx)}),
        //displaydata:            (ctx) =>  console.log('afsd')//ctx) => calcDisplay.textContent = ctx.data.join(""),
    },
     services: {}
   };
...
function Display({value}) {
    return <p id="display" className="calc_display">{value}</p>;
}
